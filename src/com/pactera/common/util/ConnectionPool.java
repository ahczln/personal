...
public class ConnectionPool {
	...
	private void createConnections(int numConnections) throws SQLException {
		...
		for (int x = 0; x < numConnections; x++) {
			...
			try {
				conn = newConnection();
				connections.addElement(new PooledConnection(conn));
			} catch (SQLException e) {
				System.out.println(" 创建数据库连接失败！ " + e.getMessage());
				throw new SQLException();
			}
			System.out.println(conn + " 数据库连接己创建 ......");
		}
	}
	...
	public synchronized void closeConnectionPool() throws SQLException {
		...
		while (enumerate.hasMoreElements()) {
			...
			System.out.println("关闭数据库连接..." + pConn.getConnection());
		}
		System.out.println("关闭数据库链接池...");
		// 置连接池为空
		connections = null;
	}
	...
	public static void main(String[] args) {
		...
		ConnectionPool connPool = new ConnectionPool(
				...
				"jdbc:oracle:thin:@192.168.17.210:1522:orcl", "pactera",
				"pactera");
		...
		try {
			...
			System.out.println("获得到的可用的数据库链接: " + conn);
